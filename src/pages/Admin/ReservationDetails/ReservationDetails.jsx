    ...
    useEffect(() => {
        ...
        if (searchQuery.trim()) {
            filtered = filtered.filter((res) =>
                res.email.toLowerCase().includes(searchQuery.toLowerCase()||res.name.toLowerCase().includes(searchQuery.toLowerCase()))
            );
        }
        ...
        if (sortConfig.key) {
            filtered = [...filtered].sort((a, b) => {
                const valueA = sortConfig.key === 'createdAt'
                    ? a[sortConfig.key]?.seconds
                    : a[sortConfig.key];
                const valueB = sortConfig.key === 'createdAt'
        ...
        <>
            ...
            <main className="w-screen h-screen bg-brown p-4 pt-52">
                <div className="container mx-auto">
                    <h1 className="text-2xl font-bold text-gray-800 mb-4 text-brown-100">Reservation Details</h1>
                                ...
                                {filteredReservations.length > 0 ? (
                                    filteredReservations.map((res) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-3 px-6 text-left">{res.outlet.title}</td>
                                            <td className="py-3 px-6 text-left">
                                                {res.createdAt
                                                    ? new Date(
